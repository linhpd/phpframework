...
class SQLQuery {
    ...
    protected $_page;
    protected $_limit;
    protected $_arrayValue = array();
    protected $_arrayValueChild = array();
    ...
    function where($field, $value) {
        $this->_extraConditions .= '`' . $this->_model . '`.`' . $field . '` = \'(?)\' AND ';
        array_push($this->_arrayValueChild, $value);
    }

    function like($field, $value) {
        $this->_extraConditions .= '`' . $this->_model . '`.`' . $field . '` LIKE \'%(?)%\' AND ';
        array_push($this->_arrayValueChild, $value);
    }
    ...
    function search() {
        ...
        if ($this->id) {
            $conditions .= '`' . $this->_model . '`.`id` = \'(?)\' AND ';
            array_push($this->_arrayValue, $id);
        }

        if ($this->_extraConditions) {
            $conditions .= $this->_extraConditions;
            array_push($this->_arrayValue, $this->_arrayValueChild);
        }
        ...
        $this->_result->execute($this->_arrayValue);
        $result = array();
        $table = array();
        $field = array();
        $tempResults = array();
                ...
                if ($this->_hM == 1 && isset($this->hasMany)) {
                    foreach ($this->hasMany as $aliasChild => $modelChild) {
                        ...
                        if ($resultChild->rowCount() > 0) {
                            ...
                            while ($rowChild = $resultChild->fetch(PDO::FETCH_NUM, PDO::FETCH_ORI_NEXT)) {
                                for ($j = 0; $j < $numOfFieldsChild; ++$j) {
                ...
                if ($this->_hMABTM == 1 && isset($this->hasManyAndBelongsToMany)) {
                    foreach ($this->hasManyAndBelongsToMany as $aliasChild => $tableChild) {
                        ...
                        if ($resultChild->rowCount() > 0) {
                            ...
                            while ($rowChild = $resultChild->rowCount()) {
                                for ($j = 0; $j < $numOfFieldsChild; ++$j) {
                                    $tempResultsChild[$tableChild[$j]][$fieldChild[$j]] = $rowChild[$j];
                                }
                                array_push($resultsChild, $tempResultsChild);
